...
class MainWidget(QWidget):
    ...
    def onBtnFocus(self):
        if self.hcam:
            self.rough_approach(200, 50, 10)
            self.fine_approach(5, 20)
            self.fine_approach(1, 10)
    ...
    def fine_approach(self, step_size, max_iteration):
        ...
        for i in range(max_iteration):
            self.send_gcommand(f"G91 G1 Z{step_size * direction} F3000")
            time.sleep(0.02)
            image = self.last_image.copy()
            gray_image = auto_focus.rgb_to_gray(image)
            FV = auto_focus.calculate_FV(gray_image)
            ...
            FV_queue[-1] = FV
            if FV_queue[0] >= FV_queue[1] and FV_queue[1] <= FV_queue[2]:
                flag = 1
                break
