...
from PyQt5.QtGui import QPixmap, QImage, QIntValidator, QPainter, QPen
...
class AutoFocus:
    ...
    def fourier_sharpness_2n(self, gray_image):
        ...
        if N == 4168:
            ...
            gray_image_cp = gray_image_cp[528:2576, 18:2066]
            print(f"size of gray_image_cp: {gray_image_cp.shape}")
        elif N == 1386:
            # Same for image that has size 2064x1386, shrink to 1024 x 1024
            gray_image_cp = gray_image_cp[533:1557, 181:1205]
            print(f"size of gray_image_cp: {gray_image_cp.shape}")
...
class VideoLabel(QLabel):
    def __init__(self, parent=None):
        ...
        self.scale_bar_length_um = 200
        self.image_physical_width_um = 4800
        self.objective_index = 0

    def setScaleBarParams(self, scale_bar_length_um, image_physical_width_um, objective_index):
        self.scale_bar_length_um = scale_bar_length_um
        self.image_physical_width_um = image_physical_width_um
    ...
    def paintEvent(self, event):
        ...
        scale_bar_length_pixels = int(self.scale_bar_length_um * label_width / self.image_physical_width_um)

        # Draw the scale bar
        x_start = int(self.width() * 0.05)  #5% from left
        y_start = int(self.height() * 0.9)
        painter.drawLine(x_start, y_start, x_start + scale_bar_length_pixels, y_start)
        painter.drawText(x_start + scale_bar_length_pixels + 5, y_start + 5, f"{self.scale_bar_length_um} μm")
...
class MainWidget(QWidget):
    ...
    def __init__(self):
        ...
        self.scan_step_x = [4.8, 2.4, 1.2, 0.48, 0.24]
        self.scan_step_y = [3.2, 1.6, 0.8, 0.32, 0.16]
        ...
        self.input_expoTime = QLineEdit()
        self.input_expoGain = QLineEdit()
        self.input_expoTime.setValidator(QIntValidator(0, 1000000))
        self.input_expoGain.setValidator(QIntValidator(100, 10000))
        self.input_expoTime.setEnabled(False)
        self.input_expoGain.setEnabled(False)
        self.cbox_auto.stateChanged.connect(self.onAutoExpo)
        self.input_expoTime.returnPressed.connect(self.onExpoTimeInput)
        self.input_expoGain.returnPressed.connect(self.onExpoGainInput)
        vlytexp = QVBoxLayout()
        vlytexp.addWidget(self.cbox_auto)
        vlytexp.addLayout(self.makeLayout(
            QLabel("Time(us):"), self.input_expoTime, self.lbl_expoTime, 
            QLabel("Gain(%):"), self.input_expoGain, self.lbl_expoGain))
        ...
        gboxobj = QGroupBox("Objective")
        self.cmb_obj = QComboBox()
        vlytobj = QVBoxLayout()
        vlytobj.addWidget(self.cmb_obj)
        gboxobj.setLayout(vlytobj)
        self.cmb_obj.addItem("5x")
        self.cmb_obj.addItem("10x")
        self.cmb_obj.addItem("20x")
        self.cmb_obj.addItem("50x")
        self.cmb_obj.addItem("100x")
        self.cmb_obj.currentIndexChanged.connect(self.onObjChanged)
        self.cmb_obj.setCurrentIndex(self.obj_index)
        ...
        self.lbl_lvar = QLabel()

        self.lbl_video = VideoLabel(self)
        self.objective_to_length_um = {
            5: (200, 4800),
            10: (100, 2400),
            20: (50, 1200),
            50: (20, 480),
        ...
        self.lcd_position_x.setDigitCount(5)
        self.lcd_position_y.setDigitCount(5)
        self.lcd_position_z.setDigitCount(8)
    ...
    def onTimer(self):
        if self.hcam:
            ...
            print(nFrame, nTime, nTotalFrame)
    ...
    def onAutoExpo(self, state):
        if self.hcam:
            ...
            self.input_expoTime.setEnabled(not state)
            self.input_expoGain.setEnabled(not state)

    def onExpoTimeInput(self):
        if self.hcam:
            new_time = int(self.input_expoTime.text())
            self.lbl_expoTime.setText(str(new_time))
            if not self.cbox_auto.isChecked():
                self.hcam.put_ExpoTime(new_time)

    def onExpoGainInput(self):
        if self.hcam:
            new_gain = int(self.input_expoGain.text())
            self.lbl_expoGain.setText(str(new_gain))
            if not self.cbox_auto.isChecked():
    ...
    def onObjChanged(self, index):
        self.obj_index = index
        objectives = [5, 10, 20, 50, 100]
        scale_bar_length_um, image_physical_width_um = self.objective_to_length_um[objectives[self.obj_index]]
        self.lbl_video.setScaleBarParams(scale_bar_length_um, image_physical_width_um, self.obj_index)
    ...
    def openCamera(self):
        ...
        if self.hcam:
            ...
            self.lbl_lvar.setText("Sharpness: 0")
            self.startCamera()
    ...
    def onBtnFocus(self):
        if self.hcam:
            ...
            print(f'Focus step size: {focus_initial_step[self.obj_index]} -> {focus_final_step[self.obj_index]}') 
            self.focus_thread = FocusThread(self, self.smooth_approach, focus_initial_step[self.obj_index], focus_final_step[self.obj_index], 40)
    ...
    def smooth_approach(self, first_step, final_step, max_iterations):
        auto_focus = AutoFocus()
        z_displacement = first_step
        sharpness = np.zeros((3,), dtype=np.float64)
        count = 0
        direction = 1
        find_peak = 0
        ...
        sharpness[-1] = auto_focus.fourier_sharpness_2n(gray_image)
        
        print(f"Initial Sharpness: {sharpness[-1]:.6f}")
        ...
        if self.hcam:
            for i in range(max_iterations):
                ...
                f_sharpness = auto_focus.fourier_sharpness_2n(gray_image)
                
                sharpness[:-1] = sharpness[1:]
                sharpness[-1] = f_sharpness
                
                print(f"Iteration: {i}, Sharpness Value: {f_sharpness:.6f}")
                count += 1
                end_time = time.time()
                print(f"Time elapsed: {end_time - start_time:.2f} s")

                if count == 1:
                    if sharpness[2] >= sharpness[1]:
                        print("first step direction correct")
                    else:
                        direction = - direction
                        print("first step direction incorrect")
                else:
                    if sharpness[1] >= sharpness[0] and sharpness[1] >= sharpness[2]:
                        if find_peak == 0:    
                            direction = - direction
    ...
    def scan_capture(self,start_point, end_point):
        ...
        file_name = f"scan_points.txt"

        with open(os.path.join(self.picture_save_folder, file_name), "w") as file:
        # 定义列宽
            col_width = 15

            # 写入表头
            header = "Y \\ X".ljust(col_width) + "".join(f"X{i}".ljust(col_width) for i in range(self.nx + 1)) + "\n"
            file.write(header)

            # 写入每行数据
            for j in range(self.ny + 1):
                row = [f"Y{j}".ljust(col_width)]  # 当前行的 Y 表头
                for i in range(self.nx + 1):
                    row.append(f"({X[i, j]:.1f}, {Y[i, j]:.1f})".ljust(col_width))  # 格式化为对齐的 (x, y)
                file.write("".join(row) + "\n")
    ...
    def show_save_path_dialog(self):
        ...
        self.scan_x = self.scan_step_x[self.obj_index]
        self.scan_y = self.scan_step_y[self.obj_index]
    ...
    def handleImageEvent(self):
        ...
        try:
            ...
            if self.lvar_refresh_count == 100:
                self.lvar_refresh_count = 0
                gray_image = auto_focus.rgb_to_gray(self.last_image)
                self.l_var = auto_focus.fourier_sharpness_2n(gray_image)

            #     gray_flat = gray_image.flatten()
            #     max_n_gray_flat = heapq.nlargest(n, gray_flat)
